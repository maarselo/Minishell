/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   executor.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: fbanzo-s <fbanzo-s@student.42barcelona.    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/24 10:39:01 by mvillavi          #+#    #+#             */
/*   Updated: 2025/11/08 23:53:09 by fbanzo-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	ft_execute_child(t_command *current_cmd, t_data *data)
{
	char	*path;
	char	**env_array;

	env_array = ft_convert_list(data);
	path = ft_find_path(current_cmd, env_array, data);
	if (!path)
		ft_error_command_not_found(current_cmd, env_array, data);
	if (execve(path, current_cmd->command, env_array) == -1)
		ft_error_command_not_found(current_cmd, env_array, data);
}

int	ft_check_wheter_continue(t_command *command, t_data *data)
{
	if (command->connector == AND_CONNECTOR && data->exit_status != 0)
		return (0);
	else if (command->connector == OR_CONNECTOR && data->exit_status == 0)
		return (0);
	return (1);
}

void	ft_wait_childs(int executed_commands, t_data *data)
{
	int	index;
	int	status;

	index = 0;
	while (index <= executed_commands)
	{
		waitpid(data->pid_array[index], &status, 0);
		if (WIFEXITED(status))
			ft_set_global_exit_status(data, WEXITSTATUS(status));
		else if (WIFSIGNALED(status))
			ft_set_global_exit_status(data, 128 + WTERMSIG(status));
		index++;
	}
}

int	ft_execute_command(bool is_last, t_command *current_command,
				int executed_commands, t_data *data)
{
	pid_t	pid;

	if (ft_isbuiltin(current_command->command[0]))
		return (ft_execute_builtin(is_last, current_command, data));
	pid = fork();
	if (pid == -1)
		return (perror("minishell:"),
			ft_set_global_exit_status(data, T_GENERAL_ERROR), 0);
	else if (pid == 0)
	{
		ft_set_signals_child_mode();
		ft_execute_child(current_command, data);
	}
	else
	{
		data->pid_array[executed_commands] = pid;
		ft_set_signals_parent_mode();
		if (current_command->connector == AND_CONNECTOR
			|| current_command->connector == OR_CONNECTOR || is_last)
		{
			ft_wait_childs(executed_commands, data);
			return (ft_check_wheter_continue(current_command, data));
		}
	}
	return (1);
}

void	ft_executor(t_data *data)
{
	int			prev_pipe;
	int			keep;
	t_command	*current_command;
	int			executed_commands;

	prev_pipe = -1;
	current_command = data->cmd;
	executed_commands = 0;
	ft_init_pid_array(data);
	while (current_command)
	{
		if (ft_manage_pipes(&prev_pipe, current_command, data->cmd, data)
			|| ft_check_heredoc(current_command, &prev_pipe, data))
			return ;
		if (ft_manage_redirections(current_command, data))
			ft_close_pipe(&prev_pipe);
		else if (current_command->command)
			keep = ft_execute_check_keep(executed_commands,
					current_command, data);
		ft_resturare_defaults_fd(data);
		if (!keep)
			break ;
		executed_commands++;
		current_command = current_command->next;
	}
}
